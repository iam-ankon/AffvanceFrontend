...
import { CheckCircle2, Zap, ShieldCheck, Flame } from 'lucide-react';
import Link from 'next/link';

const PLANS_REDIRECT = '/login?redirect=' + encodeURIComponent('/app/subscription/plans');

type BillingCycle = 'monthly' | 'yearly';

const plans = [
    { name: "Starter", monthlyPrice: 29, originalPrice: 39, tokens: "50k Tokens", desc: "For solo bloggers.", features: ["SEO Quality Score", "CMS Sync", "Originality Shield", "Weekly Growth Report"], cta: "Try Starter", popular: false },
    { name: "Pro", monthlyPrice: 79, originalPrice: 99, tokens: "150k Tokens", desc: "For publishers.", features: ["Bulk Content Mode", "Auto-Link Clusters", "Priority Search Node", "Keyword Lab Pro", "Snippet Pro"], cta: "Get Started", popular: true },
    { name: "Agency", monthlyPrice: 199, originalPrice: 249, tokens: "500k Tokens", desc: "For power teams.", features: ["Team Workspaces", "API Automation", "Expert Consult", "White-Label Hub", "Managed Loops"], cta: "Join Agency", popular: false }
];

export const PricingClient = () => {
    const [cycle, setCycle] = useState<BillingCycle>('monthly');

    const calculateYearlyMonthlyPrice = (p: number) => Math.floor((p * 10) / 12);

    return (
        <>
            {/* Monthly/Yearly Toggle */}
            <div className="flex justify-center items-center gap-6 mb-24">
                <span className={`text-sm font-black uppercase tracking-widest ${cycle === 'monthly' ? 'text-indigo-600' : 'text-slate-400'}`}>Monthly</span>
                ...
                <div className="flex items-center gap-3">
                    <span className={`text-sm font-black uppercase tracking-widest ${cycle === 'yearly' ? 'text-indigo-600' : 'text-slate-400'}`}>Yearly</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 items-end">
                            ...
                            <div className="flex flex-col items-center">
                                <div className="flex items-center justify-center gap-2 mb-2">
                                    ...
                                    <span className="text-7xl font-black text-slate-900 tracking-tighter leading-none">
                                        {cycle === 'monthly' ? plan.monthlyPrice : calculateYearlyMonthlyPrice(plan.monthlyPrice)}
                                    </span>
                                </div>
                                <div className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em] mb-4">per month</div>
                                <div className="inline-block px-5 py-2 bg-indigo-50 text-indigo-600 text-[11px] font-black uppercase tracking-widest rounded-xl border border-indigo-100">{plan.tokens}</div>
                            </div>
                        </div>
                        <ul className="space-y-5 mb-14 flex-1 border-t border-slate-50 pt-10">
                            {plan.features.map((f, j) => (
                                <li key={j} className="flex items-start gap-4 text-slate-600 text-[12px] font-bold leading-tight"><CheckCircle2 className="w-5 h-5 text-indigo-600 shrink-0" /> {f}</li>
                            ))}
                        </ul>
                        <Link href={PLANS_REDIRECT} className={`block w-full py-7 rounded-[2.5rem] font-black text-[11px] uppercase tracking-[0.3em] transition-all text-center ${plan.popular ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-200' : 'bg-slate-50 text-slate-900 hover:bg-slate-900 hover:text-white'}`}>{plan.cta}</Link>
                    </div>
                ))}
                ...
                <div className="relative p-10 rounded-[3.5rem] bg-slate-950 text-white border-2 border-indigo-500 shadow-[0_25px_60px_-15px_rgba(79,70,229,0.4)] flex flex-col transition-all hover:-translate-y-2 duration-500 overflow-hidden min-h-[750px] group lg:scale-95 origin-bottom">
                    ...
                    <div className="relative z-10 flex-1 flex flex-col">
                        ...
                        <div className="flex flex-col items-center justify-center py-14 bg-white/5 rounded-[3rem] border border-white/10 mb-10 shadow-2xl relative group-hover:bg-white/10 transition-colors">
                            ...
                            <div className="absolute -bottom-4 bg-indigo-600 px-6 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">One-Time Activation</div>
                        </div>
                        ...
                        <Link href={PLANS_REDIRECT} className="block w-full py-7 bg-white text-slate-900 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.3em] hover:bg-indigo-600 hover:text-white transition-all shadow-2xl mt-auto text-center">Get Pass for $1</Link>
                    </div>
                </div>
            </div>
        </>
